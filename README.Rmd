...
The package  **rofanova** implements the robust nonparametric functional ANOVA method (Ro-FANOVA) proposed by Centofanti et al. (2021).
Ro-FANOVA addresses the functional analysis of variance (FANOVA) problem, which aims to identify the presence of significant differences, in terms of
...
The main function is `rofanova` which implements the Ro-FANOVA method both for univariate and bi-variate functional data by using several families of loss functions. The functions `fusem` and `funmad` implement the functional equivariant M-estimator and the FuNMAD estimator, respectively.
...
```
The p-values for the siginificance of the whole model, the two main factors and the interaction are
```{r} 
print(pvalues)
```
# References
